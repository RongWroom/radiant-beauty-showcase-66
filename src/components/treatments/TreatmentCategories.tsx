...
const TreatmentCategories = () => {
  ...
  return <section className="relative min-h-[600px] overflow-hidden">
      {/* Background transitions */}
      {categories.map((category, index) => <div key={index} className={`absolute inset-0 transition-opacity duration-1000 ${index === currentIndex ? 'opacity-100' : 'opacity-0'} ${category.background}`} />)}
      ...
      <div className="relative z-10 flex items-center justify-center min-h-[600px] px-4 sm:px-6 md:px-8">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex justify-center space-x-3 mb-8">
            {categories.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-brand-silver scale-125' : 'bg-white/40 hover:bg-white/60'}`} aria-label={`Go to category ${index + 1}`} />)}
          </div>
...
Beauty Treatments</h2>
          ...
          <div className="transition-all duration-1000 transform">
            ...
            return <div key={index} className={`transition-all duration-1000 ${index === currentIndex ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8 absolute inset-0'}`}>
          ...
          <div className="flex justify-center space-x-4 mt-12">
            <button onClick={() => setCurrentIndex((currentIndex - 1 + categories.length) % categories.length)} className="w-12 h-12 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center text-white hover:bg-white/30 transition-all duration-300 hover:scale-110" aria-label="Previous category">
              ←
            </button>
            <button onClick={() => setCurrentIndex((currentIndex + 1) % categories.length)} className="w-12 h-12 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center text-white hover:bg-white/30 transition-all duration-300 hover:scale-110" aria-label="Next category">
              →
            </button>
          </div>
        </div>
