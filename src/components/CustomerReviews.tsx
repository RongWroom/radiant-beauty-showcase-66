  ...
  return <section className="relative overflow-hidden bg-gradient-to-b from-brand-off-white to-brand-white section pb-12">
      <div className="container-custom relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mb-10">
          {visibleReviews.map(review => <div key={review.id} className="backdrop-blur-sm p-6 md:p-7 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col h-full group hover:-translate-y-1 border border-brand-silver/30 animate-fade-in bg-slate-200">
              <div className="flex mb-5">
                {Array.from({
              length: review.stars
            }).map((_, index) => <Star key={index} className="h-5 w-5 fill-brand-slate-blue text-brand-slate-blue" />)}
              </div>
        ...
        <div className="text-center mb-8">
          {reviews.length > defaultCount && <Button className="bg-brand-slate-blue text-white text-base py-3 px-8 shadow-md transition-all duration-300" variant="default" onClick={() => setShowAll(s => !s)} aria-expanded={showAll}>
